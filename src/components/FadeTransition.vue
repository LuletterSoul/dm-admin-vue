  ...
  computed: {
    transitionMode: function() {
      if (this.isProgressive) {
        console.info(
          "Progressive mode must enable in-and-out props.Current computed object will be noneffective."
        );
        return;
      }
      if (this.fadeInOnly && this.fadeOutOnly) {
        console.error(
          "Fade mode can't be set at 'false' or 'true' at the same time."
        );
        return null;
    ...
    fadeDuration: function() {
      if (this.fadeInOnly) {
        ...
        console.error(
          "Fade mode must be set at out-in-progressive one of them."
        );
      }
    },
